...
def backup_data(config):
    ...
    for server in config['servers']:
        ...
        with pysftp.Connection(**connection_params) as sftp:
            logging.info(f"Connected to {hostname}, checking for startup commands if any defined")
            ...
            if sftp.exists(remote_dir):
                ...
                if latest_file:
                    ...
                    previous_files_bytes = sftp.execute(
                        f"ls -t {remote_dir}/*.gz | head -n 6 | tail -n 5"
                    )[0]
                    previous_files = previous_files_bytes.decode().strip().split("\n")
                    ...
                    if smaller_file_found:
                        error_message = f"Some recent files on {hostname} are at least 25% smaller in size than the latest file."
                        print(
